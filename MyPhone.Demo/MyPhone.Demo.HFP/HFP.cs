...
internal class HFP
{
    ...
    public static readonly Guid HFP_Id = new("0000111f-0000-1000-8000-00805f9b34fb");
    ...
    private static readonly AutoResetEvent _PendingResult = new(false);
    ...
    private static async Task<Dictionary<Guid, PhoneLine>> GetPhoneLinesAsync()
    {
        ...
        if (!await lineEnumerationCompletion.Task) throw new Exception("Phone Line Enumeration failed");

        watcher.Stop();

        var returnedLines = new Dictionary<Guid, PhoneLine>();

        foreach (var phoneLine in phoneLines.Where(phoneLine => phoneLine != null && phoneLine.Transport == PhoneLineTransport.Cellular))
        {
    ...
    public static async Task DoTest()
    {
        ...
        {
            if (s < 0 || s > devices.Count) goto select;
        ...
        if (!pDevice.IsRegistered()) pDevice.RegisterApp();
        var flag = await pDevice.ConnectAsync();
        Console.WriteLine(flag ? "Connected" : "Could not connect to PhoneLineDevice");
        var store = await PhoneCallManager.RequestStoreAsync();
        var pl = await PhoneLine.FromIdAsync(await store.GetDefaultLineAsync());
        pl.DialWithOptions(new PhoneDialOptions
            { AudioEndpoint = PhoneAudioRoutingEndpoint.Bluetooth, Number = "18589846271" });
    ...
    public static async Task Do()
    {
        ...
        var flag = await HFPConnect(devices[s].Id);
        if (!flag) goto select;
        ReceiveLoop(_reader);
    ...
    public static async Task OnMessageReceived(string message)
    {
        ...
        {
            if (uint.TryParse(message.Substring(6), out var i)) AG_Supported_Features = i;
        }
        else if (message.StartsWith("+BCS:"))
        {
            if (uint.TryParse(message.Substring(5), out var i))
            {
